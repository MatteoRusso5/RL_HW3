...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            ...
            default_value='true',
            description='Start robot with the camera.',
        )
    )
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            ...
            default_value='true',
            description='Set to true to use aruco.world, false to use sphere.world',
        )
    )
   ...
   # declared_arguments.append(DeclareLaunchArgument('gz_args', default_value='-r -v 1 empty.sdf',
    ...
    #  worlds
    aruco_world = PathJoinSubstitution(
        [FindPackageShare(description_package), 'gazebo/worlds', 'aruco.world']
    )
    sphere_world = PathJoinSubstitution(
        [FindPackageShare(description_package), 'gazebo/worlds', 'sphere.world']
    )

    # configurations of 'gz_args'
    aruco_gz_args = DeclareLaunchArgument(
        'gz_args',
        default_value=[aruco_world, ' -r -v 1'],
        description='Arguments for gz_sim with aruco.world',
        condition=IfCondition(LaunchConfiguration('use_aruco'))
    )
    sphere_gz_args = DeclareLaunchArgument(
        'gz_args',
        default_value=[sphere_world, ' -r -v 1'],
        description='Arguments for gz_sim with sphere.world',
        condition=UnlessCondition(LaunchConfiguration('use_aruco'))
    )
